...
void
putbe16 (void *pvoid, uint16_t val)
{
  unsigned char *p = (unsigned char *) pvoid;
...
void
putbe32 (void *pvoid, uint32_t val)
{
  unsigned char *p = (unsigned char *) pvoid;
...
main (int argc, char **argv)
{
  ...
  uint32_t elfmagicword = getbe32 (&header.ident0);
  ...
  uint32_t phoff = getbe32 (&header.phoff);
  uint16_t phnum = getbe16 (&header.phnum);
  ...
  elfphentry entries[phnum + 2];
  offset_size_pair offsetsizes[phnum];
      ...
      if (filesz)
        {
          ...
          fprintf (stdout,
              "Writing segment 0x%08X to 0x%08X (%d bytes) - %x %s\n",
              getbe32 (&p->vaddr), ftell (fout), filesz,
              getbe32 (&p->memsz), (strip == getbe32 (&p->vaddr)
                  && strip != 0) ? "- Stripped" : "");

          unsigned char data[filesz];

          if (fread (data, filesz, 1, fin) != 1
              || fwrite (data, filesz, 1, fout) != 1)
            {
